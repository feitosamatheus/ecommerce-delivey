...
namespace Ecommerce.MVC.Areas.Admin.Controllers
{
    ...
    public class CozinhaController : Controller
    {
        ...
        public async Task<IActionResult> Prontos(DateTime? dataInicio, DateTime? dataFim, string nomeCliente, string numeroPedido)
        {
            ...
            var inicioUtc = dataInicio.Value.Date.ToUniversalTime();
            var fimUtc = dataFim.Value.Date.AddDays(1).ToUniversalTime();
        ...
        public async Task<IActionResult> DetalhesPagamento(Guid id)
        {
            // Buscar o pagamento associado ao pedido
            var pagamento = await _db.PedidoPagamentos
                .AsNoTracking()
                .Include(p => p.Pedido)
                .FirstOrDefaultAsync(p => p.PedidoId == id);  // Busca pelo ID do Pedido
            ...
            // Retorna a partial view com os detalhes do pagamento
            return PartialView("_DetalhesPagamento", pagamento);
        }
